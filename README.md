...
**macOS：**
```bash
curl -L "https://github.com/fuxm0818/coi/releases/latest/download/coi-macos" -o /usr/local/bin/coi
...
**Linux：**
```bash
curl -L "https://github.com/fuxm0818/coi/releases/latest/download/coi-linux" -o /usr/local/bin/coi
chmod +x /usr/local/bin/coi
coi --help
```
...
模型文件已随项目打包在 `model/` 目录中，开箱即用，无需额外下载。
...
> 注意：首次运行时会检测 `model/` 目录，如已有模型则直接使用，否则会提示下载。
...
├── coi (或 coi.exe)    # 可执行文件
├── model/              # 嵌入模型目录（已随项目提供）
│   └── model.onnx      # ONNX 格式模型（约 90MB）
└── coi_data/          # [可选] 用户数据（如需保留历史问答）
```

**分发说明：**
- 模型文件已打包在 `model/` 目录，无需额外下载
- `coi_data/` 是运行时生成的用户数据，包含配置、向量库、FQA 问答库
...
- `model/` 目录包含嵌入模型文件，已包含在版本控制中（方便离线分发）
