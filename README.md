...
$coiDir = "$env:LOCALAPPDATA\coi"
New-Item -ItemType Directory -Force -Path $coiDir | Out-Null
curl.exe -L "https://ghproxy.net/https://github.com/fuxm0818/coi-qa/releases/latest/download/coi-x86_64-pc-windows-msvc.zip" -o "$coiDir\coi.zip"
Expand-Archive -Path "$coiDir\coi.zip" -DestinationPath $coiDir -Force
& "$coiDir\coi.exe" --help
```
...
tar -xzf /tmp/coi.tar.gz -C /usr/local/bin/
...
```bash
git clone https://github.com/fuxm0818/coi-qa.git
cd coi
cargo build --release
```
