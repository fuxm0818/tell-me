...
tell-me 让你指定一个本地文档文件夹，自动构建向量知识库，通过命令行提问即可检索文档内容。全程不联网、不调用任何云端大模型，所有数据留在本地。
...
- 🗂️ **数据透明** — 所有数据存放在可见的 `tell_me_data/` 文件夹，删除即重置
...
tar -xzf /tmp/tell-me.tar.gz -C /usr/local/bin/
chmod +x /usr/local/bin/tell-me
tell-me --help
```
...
```bash
git clone https://github.com/fuxm0818/tell-me.git
cd tell-me
cargo build --release
```

编译产物在 `target/release/tell-me`。
...
```bash
xattr -d com.apple.quarantine ./tell-me-macos
```
...
首次运行时，程序会自动将模型提取到 `tell_me_data/model/` 目录，之后运行直接使用已提取的模型。
...
```bash
tell-me init /path/to/your/documents
```
...
```bash
tell-me add-fqa "项目的部署流程是什么？" "先执行 build，再通过 Docker 部署到生产环境"
```
...
所有程序数据存放在可执行文件同级目录下的 `tell_me_data/` 文件夹：
...
直接删除 `tell_me_data/` 文件夹等同于执行 `tell-me clear`。
...
```
dist/
├── tell-me-macos           # macOS 通用版（Intel + M芯片都能运行）
├── tell-me-windows.exe     # Windows 64位版本
└── tell-me-linux           # Linux 64位版本
```
...
```
分发包/
└── tell-me (或 tell-me.exe)    # 可执行文件（模型已嵌入，约 110MB）
```

**分发说明：**
- 模型已嵌入可执行文件，分发时无需额外拷贝 `model/` 目录
- 首次运行时程序会自动提取模型到 `tell_me_data/model/`
- `tell_me_data/` 是运行时生成的用户数据目录，包含配置、向量库、FQA 问答库
- 直接删除 `tell_me_data/` 等同于执行 `tell-me clear`
...
│   ├── tell-me-macos           # macOS 通用版（Intel + M芯片）
│   ├── tell-me-windows.exe     # Windows 版
│   └── tell-me-linux           # Linux 版
│
│
...
- `tell_me_data/` 是运行时生成的用户数据目录，不在版本控制中
